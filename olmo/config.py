...
#
# Modifications: Added DynamicLoopConfig for dynamic loop attention support.
...
class DynamicLoopConfig:
    """
    Configuration for dynamic layer selection and head pruning with loop attention.
    
    动态选层选头配置：
    - 训练开始时是 vanilla 模式
    - 根据注意力熵动态选择层和头进行 loop
    - 选层规则：排除 layer_0,按熵排序取 top_k,从最深的开始选
    - 选头规则：在选中的层内，选熵最高的 num_keep_heads 个头保持 loop
    """
    
    # === 是否启用动态 loop ===
    enabled: bool = False
    """
    Whether to enable dynamic loop selection.
    If False, the model trains in vanilla mode without any loop layers.
    """
    
    # === 选层参数 ===
    top_k_layers: int = 3
    """
    Number of top entropy layers to consider for selection.
    从熵排序的 top_k 中选择最深的层。
    """
    
    max_selected_layers: int = 3
    """
    Maximum number of different layers that can be selected for loop.
    选够这么多层后，结构固定，不再选新层。
    """
    
    max_loop_depth: int = 2
    """
    Maximum loop depth per layer.
    每层最大 loop 深度。
    """
    ...
    """
    Number of attention heads to keep per layer (highest entropy heads).
    在选中的层内，保留熵最高的这么多个头。
    """
    ...
    window_size: int = 250
    """
    Window size (in steps) for each selection/deepening phase.
    每次选层或加深 loop 后的窗口期长度。
    """
    
    first_selection_step: int = 250
    """
    Step at which the first layer selection occurs.
    第一次选层发生的 step。在此之前是 vanilla 训练。
    """
    
    # === 衰减参数 ===
    decay_type: str = "linear"
    """
    Type of decay for non-kept heads: "linear", "cosine", or "exponential".
    非保留头的衰减类型。
    """
    ...
    ema_decay: float = 0.5
    """
    EMA decay factor for entropy accumulation.
    0.0 = instant entropy (no history)
    0.5 = balanced (recommended)
    0.9 = highly smoothed
    """
    
    # === 排除层 ===
    exclude_layers: List[int] = field(default_factory=lambda: [0])
    """
    Layers to exclude from selection (e.g., layer 0).
    排除的层，不参与选层。
    """

    # === 结构固定后的行为 ===
    compute_entropy_after_fixed: bool = False
    """
    Whether to continue computing entropy after structure is fixed.
    如果为 True, 结构固定后继续计算被选中 loop 层的注意力熵（用于监控）
    如果为 False, 结构固定后停止所有熵计算（节省计算）
    """
    ...
    def __post_init__(self):
        ...
        if self.top_k_layers <= 0:
            raise ValueError(f"top_k_layers must be positive, got {self.top_k_layers}")
        
        if self.max_selected_layers <= 0:
            raise ValueError(f"max_selected_layers must be positive, got {self.max_selected_layers}")
        
        if self.max_loop_depth < 1:
            raise ValueError(f"max_loop_depth must be at least 1, got {self.max_loop_depth}")
        
        if self.num_keep_heads <= 0:
            raise ValueError(f"num_keep_heads must be positive, got {self.num_keep_heads}")
        
        if self.window_size <= 0:
            raise ValueError(f"window_size must be positive, got {self.window_size}")
        
        if self.first_selection_step < 0:
            raise ValueError(f"first_selection_step must be non-negative, got {self.first_selection_step}")
        
        if self.decay_type not in ("linear", "cosine", "exponential"):
...
class ModelConfig(BaseConfig):
    ...
    dynamic_loop_config: Optional[DynamicLoopConfig] = None
    """
    Configuration for dynamic layer selection and loop attention.
    If None or enabled=False, the model trains in vanilla mode.
    """
