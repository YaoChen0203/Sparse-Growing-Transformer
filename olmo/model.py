...
Modified for SGT:
- Added dynamic loop attention support
- Added dynamic layer scheduler integration
- Added entropy-based layer/head selection
...
from .dynamic_layer_scheduler import DynamicLayerScheduler
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        # === Added: Dynamic Loop support ===
        # Each block pre-creates LoopAttention, but only selected layers will actually execute loop
        self.loop_attention = None
        if (config.dynamic_loop_config is not None and
            config.dynamic_loop_config.enabled):
            self.loop_attention = LoopAttention(
                cfg=config,
                cache=cache,
                ref_block=self,
                layer_idx=layer_id,
                max_loop_depth=config.dynamic_loop_config.max_loop_depth,
                num_keep_heads=config.dynamic_loop_config.num_keep_heads,
                decay_type=config.dynamic_loop_config.decay_type,
                ema_decay=config.dynamic_loop_config.ema_decay,
            )
            # Set whether to continue computing entropy after structure is fixed
            self.loop_attention.set_compute_entropy_after_fixed(
                config.dynamic_loop_config.compute_entropy_after_fixed
            )
    ...
    def forward(
        ...
        # =====================================================================
        # Added: Compute first forward attention weights for entropy statistics
        # Key: This is computed before actual attention forward, so it's the true "first forward"
        # Only compute attention weights for the last query to save computation and memory
        # =====================================================================
        if (self.training and 
            self.loop_attention is not None and 
            not (self.loop_attention.structure_fixed and 
                 not self.loop_attention.compute_entropy_after_fixed)):
            with torch.no_grad():
                first_attn_weights_last = self._compute_last_attention_weights_for_entropy(q, k)
                self.loop_attention.accumulate_first_forward_entropy(first_attn_weights_last)
        ...
        # =====================================================================
        # Added: Dynamic Loop layer (after attention residual connection, before FF)
        # Note: loop_attention.forward() only executes additional loops
        # If current_loop_depth=0, it returns input directly without any computation
        # =====================================================================
        if self.loop_attention is not None:
    ...
    # =============================================================================
    # Added: Entropy computation helper method in OLMoSequentialBlock
    # =============================================================================
    def _compute_last_attention_weights_for_entropy(
        self,
        q: torch.Tensor,
        k: torch.Tensor,
    ) -> torch.Tensor:
        """
        Compute attention weights for the last query position (for entropy statistics only)

        Args:
            q: query tensor (B, T, D) - after QKV projection
            k: key tensor (B, T, D_kv)

        Returns:
            attn_weights_last: (B, H, T) attention weights for the last query
        """
        ...
        # Optional Q/K layer norm
        if self.q_norm is not None and self.k_norm is not None:
            q = self.q_norm(q).to(dtype=dtype)
            k = self.k_norm(k).to(dtype=dtype)
        
        # reshape: (B, T, D) -> (B, H, T, Hd)
        ...
        # Compute attention weights: (B, H, 1, Hd) x (B, H, Hd, T) -> (B, H, 1, T)
        attn_weights = torch.matmul(q_last, k.transpose(-2, -1)) / math.sqrt(head_dim)
                
        attn_weights = F.softmax(attn_weights, dim=-1)  # (B, H, 1, T)
        attn_weights = attn_weights.squeeze(2)  # (B, H, T)
...
class OLMo(nn.Module):
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if (config.dynamic_loop_config is not None and
            ...
            # Collect LoopAttention from all layers
            loop_layers = {}
            for block in self.transformer.blocks:
                if hasattr(block, 'loop_attention') and block.loop_attention is not None:
                    loop_layers[block.layer_id] = block.loop_attention

            if loop_layers:
                self.dynamic_scheduler = DynamicLayerScheduler(
                    config=config.dynamic_loop_config,
                    n_layers=config.n_layers,
                    loop_layers=loop_layers,
                )
                log.info(f"Dynamic layer scheduler initialized with {len(loop_layers)} loop layers")
    ...
    # =========================================================================
    # Dynamic Loop related methods
    # =========================================================================

    def get_loop_layers(self) -> Dict:
        """Get all LoopAttention layers"""
        loop_layers = {}
        for block in self.transformer.blocks:
            if hasattr(block, 'loop_attention') and block.loop_attention is not None:
                loop_layers[block.layer_id] = block.loop_attention
    ...
    def step_dynamic_loop(self, global_step: int) -> Dict:
        """
        Execute one step of dynamic loop scheduling

        This method should be called after optimizer.step() in each training step.

        Process:
        1. Finalize entropy accumulation (micro-batch average)
        2. Distributed sync of entropy values
        3. Scheduler executes layer selection/depth increase logic
        ...
        # 1. Finalize entropy accumulation (average across micro-batches)
        self.dynamic_scheduler.finalize_all_entropies()

        # 2. Distributed sync
        self.dynamic_scheduler.sync_entropies()

        # 3. Scheduler step
        action_info = self.dynamic_scheduler.step(global_step)

        return action_info
    
    def get_dynamic_loop_metrics(self) -> Dict[str, float]:
        """Get dynamic loop related metrics for logging"""
        if self.dynamic_scheduler is None:
            return {}
        return self.dynamic_scheduler.get_metrics()
    
    def get_dynamic_loop_state(self) -> Dict:
        """
        Get complete dynamic loop state for checkpoint saving

        Returns:
            State dictionary containing scheduler state and all loop layer states
        """
        if self.dynamic_scheduler is None:
            return {"enabled": False}
        return self.dynamic_scheduler.get_scheduler_state()
    
    def load_dynamic_loop_state(self, state_dict: Dict):
        """
        Load dynamic loop state from checkpoint
        ...
        if self.dynamic_scheduler is None:
            return
        if not state_dict.get("enabled", True):
            return
        self.dynamic_scheduler.load_scheduler_state(state_dict)
    
    def set_inference_mode(self):
        """
        Set model to inference mode

        Should be called after loading checkpoint and before inference.
        This will:
        1. Fix structure for all loop layers
        2. Stop entropy computation
        3. Ensure decay coefficients are correct
        """
        if self.dynamic_scheduler is not None:
            self.dynamic_scheduler.set_inference_mode()
    
    def get_loop_summary(self) -> str:
        """
        Get summary of loop structure (for logging)

        Returns:
            Human-readable summary string
        """
        if self.dynamic_scheduler is None:
            return "Dynamic loop not enabled"
        
        state = self.dynamic_scheduler.get_scheduler_state()
        lines = [
            f"Phase: {state['phase']}",
            f"Selected layers: {state['selected_layers']}",
        ]
        
        for layer_idx in state['selected_layers']:
            if layer_idx in self.dynamic_scheduler.loop_layers:
                layer = self.dynamic_scheduler.loop_layers[layer_idx]
                info = layer.get_state_info()
                lines.append(
                    f"  Layer {layer_idx}: loop_depth={info['current_loop_depth']}, "
                    f"kept_heads={info['kept_heads']}"
                )
