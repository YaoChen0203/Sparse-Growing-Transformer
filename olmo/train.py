...
class Trainer:
    ...
    # ------------------------------------------------------------------------------
    # 添加辅助方法 获取实际的模型对象（去除 FSDP/DDP 包装）
    # ------------------------------------------------------------------------------
    def get_actual_model(self) -> OLMo:
        if isinstance(self.dist_model, (DDP, FSDP)):
            return self.dist_model.module
        else:
            return self.model
    ...
    def trainer_state_dict(self) -> Dict[str, Any]:
        ...
        # === 新增：保存动态 loop 状态 ===
        if (self.cfg.model.dynamic_loop_config is not None and 
            self.cfg.model.dynamic_loop_config.enabled):
            actual_model = self.get_actual_model()
            state["dynamic_loop_state"] = actual_model.get_dynamic_loop_state()
    ...
    def load_trainer_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        # === 新增: 加载动态 loop 状态 ===
        if (self.cfg.model.dynamic_loop_config is not None and 
            self.cfg.model.dynamic_loop_config.enabled and
            "dynamic_loop_state" in state_dict):
            actual_model = self.get_actual_model()
            actual_model.load_dynamic_loop_state(state_dict["dynamic_loop_state"])
            log.info(f"Loaded dynamic loop state: {actual_model.get_loop_summary()}")
    ...
    def train_step(self, batch: Dict[str, Any], reduce_global_loss: bool = True) -> Dict[str, float]:
        ...
        if (self.cfg.model.dynamic_loop_config is not None and 
            self.cfg.model.dynamic_loop_config.enabled):
            actual_model = self.get_actual_model()
            loop_action = actual_model.step_dynamic_loop(self.global_step)
                ...
                for batch in self.train_loader:
                    ...
                    if (
                        ...
                        if (self.cfg.model.dynamic_loop_config is not None and 
                            self.cfg.model.dynamic_loop_config.enabled):
                            actual_model = self.get_actual_model()
                            loop_metrics = actual_model.get_dynamic_loop_metrics()
